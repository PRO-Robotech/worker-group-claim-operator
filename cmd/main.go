...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "dfe316c8.cluster.x-k8s.io",
	...
	if err := (&controller.WorkerGroupClaimReconciler{
		Client:   mgr.GetClient(),
		Scheme:   mgr.GetScheme(),
		Recorder: mgr.GetEventRecorderFor("workergroupclaim-controller"),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "WorkerGroupClaim")
		os.Exit(1)
	}
	if err := mgr.Add(controller.NewOrphanCleaner(
		mgr.GetClient(),
		mgr.GetEventRecorderFor("orphan-cleaner"),
	)); err != nil {
		setupLog.Error(err, "unable to add orphan cleaner")
		os.Exit(1)
	}
